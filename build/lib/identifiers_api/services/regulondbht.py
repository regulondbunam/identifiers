...
def _get_all_ontologies_related_ids_from(regulondb_version, regulondb_database):
    ...
    for collection_name in ["ontologies", "terms"]:
        ...
        htdb_identifiers[collection_name] = collection_identifiers.copy()
    return htdb_identifiers


def _get_identifiers_from(organism, regulondb_version, regulondb_database):
    htdb_identifiers = {}
    collections_names = list(RDB_HT_SUBCLASS_ACRONYMS.keys())
...
def get_all_identifiers(regulondb_version, organism, regulondb_database="regulondbht"):
    htdb_identifiers = _get_identifiers_from(organism, regulondb_version, regulondb_database)
    htdb_identifiers.update(_get_all_ontologies_related_ids_from(regulondb_version, regulondb_database))
    return htdb_identifiers


def get_identifiers_by(type, organism=None, ontology_name=None, regulondb_database="regulondbht"):
    collection_identifiers = {}
